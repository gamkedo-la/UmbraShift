Shader "Unlit/TVStatic"
{
    Properties
    {
        _MainTex ("Texture", 2D) = "white" {}
    }
    SubShader
    {
        Tags { "RenderType"="Opaque" }
        LOD 100

        Pass
        {
            CGPROGRAM
            #pragma vertex vert
            #pragma fragment frag
            // make fog work
            #pragma multi_compile_fog

            #include "UnityCG.cginc"

            struct appdata
            {
                float4 vertex : POSITION;
                float2 uv : TEXCOORD0;
            };

            struct v2f
            {
                float2 uv : TEXCOORD0;
                UNITY_FOG_COORDS(1)
                float4 vertex : SV_POSITION;
            };

            sampler2D _MainTex;
            float4 _MainTex_ST;

			float3 mod289(float3 x)
			{
				return x - floor(x * (1.0 / 289.0)) * 289.0;
			}

			float2 mod289(float2 x)
			{
				return x - floor(x * (1.0 / 289.0)) * 289.0;
			}

			float3 permute(float3 x)
			{
				return mod289(((x*34.0) + 1.0)*x);
			}

			float snoise(float2 v)
			{
				const float4 C = float4(
					0.211324865405187,  // (3.0-sqrt(3.0))/6.0
					0.366025403784439,  // 0.5*(sqrt(3.0)-1.0)
					-0.577350269189626, // -1.0 + 2.0 * C.x
					0.024390243902439); // 1.0 / 41.0

				// First corner
				float2 i  = floor(v + dot(v, C.yy));
				float2 x0 = v - i + dot(i, C.xx);

				// Other corners
				float2 i1;
				i1 = (x0.x > x0.y) ? float2(1.0, 0.0) : float2(0.0, 1.0);
				float4 x12 = x0.xyxy + C.xxzz;
				x12.xy -= i1;

				// Permutations
				i = mod289(i);
				float3 p = permute(permute(i.y + float3(0.0, i1.y, 1.0))
					+ i.x + float3(0.0, i1.x, 1.0));

				float3 m = max(0.5 - float3(dot(x0, x0), dot(x12.xy, x12.xy), dot(x12.zw, x12.zw)), 0.0);
				m = m * m;
				m = m * m;

				// Gradients: 42 points uniformly over a line, mapped onto a diamond.
				// The ring size 17*17 = 289 is close to a multiple of 41 (41*7 = 287

				float3 x = 2.0 * frac(p * C.www) - 1.0;
				float3 h = abs(x) - 0.5;
				float3 ox = floor(x + 0.5);
				float3 a0 = x - ox;

				// Normalize gradients implicitly by scaling m
				// Approximation of: m *= inversesqrt(a0*a0 + h*hh)
				m *= 1.79284291400159 - 0.85373472095314 * (a0*a0 + h * h);

				// Compute final noise value at P
				float3 g;
				g.x = a0.x * x0.x + h.x * x0.y;
				g.yz = a0.yz * x12.xz + h.yz * x12.yw;
				return 130.0 * dot(m, g);
			}
			
			float staticV(float2 uv)
			{
				float iTime = _Time.y;
				float staticHeight = snoise(float2(9.0, iTime*1.2 + 3.0))*0.3 + 5.0;
				float staticAmount = snoise(float2(1.0, iTime*1.2 - 6.0))*0.1 + 0.3;
				float staticStrength = snoise(float2(-9.75, iTime*0.6 - 3.0))*2.0 + 2.0;
				return (1.0 - step(snoise(float2(5.0*pow(iTime, 2.0) + pow(uv.x*7.0, 1.2), pow((fmod(iTime, 100.0) + 100.0)*uv.y*0.3 + 3.0, staticHeight))), staticAmount))*staticStrength;
			}

            v2f vert (appdata v)
            {
                v2f o;
                o.vertex = UnityObjectToClipPos(v.vertex);
                o.uv = TRANSFORM_TEX(v.uv, _MainTex);
                UNITY_TRANSFER_FOG(o,o.vertex);
                return o;
            }

            fixed4 frag (v2f i) : SV_Target
            {
                // sample the texture
				float2 uv = i.uv;
                fixed4 col = tex2D(_MainTex, uv);

				float iTime = _Time.y;
				
				float jerkOffset = (1.0 - step(snoise(float2(iTime*1.3, 5.0)), 0.8)) * 0.05;
				float fuzzOffset = snoise(float2(iTime*15.0, uv.y*80.0))*0.003;
				float largeFuzzOffset = snoise(float2(iTime*1.0, uv.y*25.0))*0.004;

				float horzFuzzOpt = 1.0;
				float vertMovementOn = 1.0;

				float xOffset = (fuzzOffset + largeFuzzOffset) * horzFuzzOpt;

				float vertJerkOpt = 1.0;
				float vertJerk = (1.0 - step(snoise(float2(iTime*1.5, 5.0)), 0.6))*vertJerkOpt;
				float vertJerk2 = (1.0-step(snoise(float2(iTime*5.5,5.0)),0.2))*vertJerkOpt;
				float yOffset = abs(sin(iTime)*4.0)*vertMovementOn + vertJerk * vertJerk2*0.3;
				float y = fmod(uv.y + yOffset, 1.0);

				float staticVal = 0.0;

				//for (float _y = -1.0; _y <= 1.0; _y += 1.0) {
				//	float maxDist = 5.0 / 200.0;
				//	float dist = _y / 200.0;
				//	staticVal += staticV(float2(uv.x, uv.y + dist))*(maxDist - abs(dist))*1.5;
				//}

				float x;
				float rgbOffsetOpt = 1.0;

				x = uv.x + xOffset - 0.01*rgbOffsetOpt;
				float red = tex2D(_MainTex, float2(x, y)).r + staticVal;
				x = uv.x + xOffset;
				float green = tex2D(_MainTex, float2(x, y)).g + staticVal;
				x = uv.x + xOffset + 0.01*rgbOffsetOpt;
				float blue = tex2D(_MainTex, float2(x, y)).b + staticVal;
				//float red = col.r + staticVal;
				//float green = col.g + staticVal;
				//float blue = col.b + staticVal;

				half4 color = half4(red, green, blue, col.a);

				float scanlinesOpt = 1.0;
				float scanline = sin(uv.y * 800.0)*0.04*scanlinesOpt;
				color -= scanline;

                // apply fog
                UNITY_APPLY_FOG(i.fogCoord, color);
                return color;
            }
            ENDCG
        }
    }
}
